...
impl SignalGen {
    ...
    pub fn new(f: Box<Fn(f64) -> Complex64>) -> SignalGen {
        SignalGen{gen: f}
    }
...
pub fn complex(freq: f64, offset: f64) -> SignalGen {
    let w = 2.0*PI*freq;
    SignalGen::new(Box::new(move |i| Complex::new(0., w*(i + offset/2.)).exp()) )
}
...
pub fn sine(freq: f64, offset: f64) -> SignalGen {
    let w = 2.0*PI*freq;
    SignalGen::new(Box::new(move |i| Complex::new(f64::sin(w*(i + offset/2.)), 0.)) )
}
...
pub fn cosine(freq: f64, offset: f64) -> SignalGen {
    let w = 2.0*PI*freq;
    SignalGen::new(Box::new(move |i| Complex::new(f64::cos(w*(i + offset/2.)), 0.)))
}
