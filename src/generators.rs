...
pub struct SignalGen {
    // TODO: Use new impl Trait syntax to avoid a Box allocation here
...
pub fn complex(freq: f64, offset: f64) -> SignalGen {
    let w = 2.0 * PI * freq;
    SignalGen {
        gen: Box::new(move |i| Complex::new(0., w * (i + offset / 2.)).exp()),
    }
}
...
pub fn sine(freq: f64, offset: f64) -> SignalGen {
    let w = 2.0 * PI * freq;
    SignalGen {
        gen: Box::new(move |i| Complex::new(f64::sin(w * (i + offset / 2.)), 0.)),
    }
}
...
pub fn cosine(freq: f64, offset: f64) -> SignalGen {
    let w = 2.0 * PI * freq;
    SignalGen {
        gen: Box::new(move |i| Complex::new(f64::cos(w * (i + offset / 2.)), 0.)),
    }
}
...
pub fn triangle(freq: f64) -> SignalGen {
    let w = 2.0 * freq;
    SignalGen {
        gen: Box::new(move |i| Complex::new((w * (i + 0.5)) % 2. - 1., 0.)),
    }
}
